...
def retry(timeout: int = 10, max_tries: int = 3, delay: int = 1, backoff: int = 2):
    ...
    an exception or exceeds the timeout.
    ...
    Raises:
        TimeoutError: When max tries is reached and last try timed out.
        ...
        def f2(*args, **kwargs):
            ...
            for tries_remaining in tries:
                ...
                thread = threading.Thread(
                    target=lambda: run_with_result(function, args, kwargs, result, exception)
                )
                thread.start()
                thread.join(timeout)
                ...
                if thread.is_alive():
                    ...
                    if tries_remaining > 0:
                        logger.warning(
                            f"Function call exceeded {timeout} seconds. Retrying..."
                        )
                        mydelay *= backoff
                        time.sleep(mydelay)
                        continue
                    else:
                        raise TimeoutError("Function call timed out after all retries.")

                if exception[0]:
                    if tries_remaining > 0:
                        logger.warning(
                            f"Function raised an exception: {exception[0]}. Retrying..."
                        )
                        mydelay *= backoff
                        time.sleep(mydelay)
                        continue
...
def run_with_result(function, args, kwargs, result_container, exception_container):
    """Helper to run a function and store results/exceptions."""
    try:
...
def run_r_script(script: str, timeout: int = 30, max_tries: int = 3):
    ...
    # Retry logic for executing R code
    @retry(stop_max_delay=timeout * 1000, stop_max_attempt_number=max_tries)
    def execute(script):
        # Ensure paths are formatted correctly for R (replace \ with / or \\)
        script = script.replace("\\", "/")  # Replace backslashes with forward slashes

        # Run the R script
        result = subprocess.run(
            [r_binary_path, "--vanilla", "--no-echo"],
            input=script,  # Pass script as a string, not bytes
            stderr=subprocess.PIPE,
            stdout=subprocess.PIPE,
            text=True  # Ensure that output is captured as a string
        )
    ...
    try:
        # Execute the R script with retry logic
        result = execute(script)
        logger.debug(f"R script executed successfully:\n{result.stdout}")
...
#                 # oldsignal = signal.signal(signal.SIGABRT, _handle_timeout)
...
#                     # signal.signal(signal.SIGABRT, oldsignal)
